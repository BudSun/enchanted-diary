// =====================================================================
// Cloudflare Worker — the production version of server.py
// Static files (index.html, fonts/, src/) are served as Worker assets.
...
// Rate limit per IP: 20/hour, in-memory (Workers isolate-local).
// For production scale, swap for Durable Objects / KV.
const RATE_MAX = 20;
...
async function upstreamPost(path, body, apiKey) {
  const resp = await fetch(`https://${PROVIDER.base}${path}`, {
    method: 'POST',
    headers: {
...
// Static asset MIME helper (kv-asset-handler maps by extension already,
// this is a safety net for unusual extensions).
const MIME = {
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/api/oracle' && request.method === 'POST') {
      return handleOracle(request, ip);
    }

    // ---- Static assets via Workers Assets ----
    try {
      // Workers Assets mounts at ASSETS binding (configured in wrangler.toml)
      if (env.ASSETS) {
        return await env.ASSETS.fetch(request);
      }
      // Fallback to kv-asset-handler if running locally without assets binding
      return await getAssetFromKV(
        { request, waitUntil: ctx.waitUntil.bind(ctx) },
        { mapRequestToAsset: (r) => mapRequest(r) },
      );
    } catch (e) {
...
// =============================================================
// /api/oracle: OCR + chat, returns SSE stream
// =============================================================
async function handleOracle(request, ip) {
  // Rate limit
  ...
  try {
    ...
    const resp = await upstreamPost('/v1/chat/completions', ocrBody, apiKey);
    if (resp.status !== 200) {
  ...
  // ---- Chat (SSE) — pass upstream Response directly ----
  try {
    ...
    const resp = await upstreamPost('/v1/chat/completions', chatBody, apiKey);
    if (resp.status !== 200) {
      const txt = await resp.text();
      return json(502, { error: `chat upstream ${resp.status}`, detail: txt.slice(0, 200) });
    }
    // Pass through SSE headers + body
